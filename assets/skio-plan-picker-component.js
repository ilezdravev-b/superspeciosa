...
export class SkioPlanPicker extends LitElement {
  ...
  sellingPlanDropdown(group) {
    return html`
      <select
        class="selling-plan-dropdown ${
          this.getAvailableSellingPlans(group).length == 1
            ? "selling-plan-dropdown--one"
            : ""
        }"
        @change=${(e) => {
            ...
            ? this.getAvailableSellingPlans(group).map(
                (selling_plan) =>
                  html` <option ?selected=${
                    group.selected_selling_plan.id === selling_plan.id
                  } value=${selling_plan.id}>${selling_plan.name}</option> `
              )
            : ""
        }
      </select>
  ...
  setupMode() {
    return html`
      <div class="setupMode">
        ...
        <div class="detection">${
          this.product ? this.validIcon() : this.invalidIcon()
        } Product</div>
        ...
        <div class="detection">${
          this.form ? this.validIcon() : this.invalidIcon()
        } Form</div>

        <div class="detection">${
          this.variantInput ? this.validIcon() : this.invalidIcon()
        } Variant Input</div>

        <div class="detection">
          ${
            this.variantChanged ? this.validIcon() : this.invalidIcon()
...
<path d="M10.25 0.218319C9.9073 -0.0932525 9.37427 -0.0688344 9.06104 0.275563L3.92434 5.93183L1.44273 3.40898C1.11515 3.0772 0.582966 3.07215 0.25203 3.39888C-0.079755 3.72476 -0.0848087 4.25779 0.241923 4.58957L3.34834 7.74737C3.5075 7.90904 3.72305 7.99915 3.94872 7.99915C3.95377 7.99915 3.95967 7.99915 3.96473 8C4.19714 7.99495 4.4161 7.89558 4.57186 7.7238L10.3072 1.40821C10.6197 1.06292 10.5944 0.530739 10.25 0.218319Z" fill="#98B64B"/>
</svg>
              ${this.options?.freeShipping}
            </p>
          </div>
          <div class="group-option-item">
            <p>
...
<path d="M10.25 0.218319C9.9073 -0.0932525 9.37427 -0.0688344 9.06104 0.275563L3.92434 5.93183L1.44273 3.40898C1.11515 3.0772 0.582966 3.07215 0.25203 3.39888C-0.079755 3.72476 -0.0848087 4.25779 0.241923 4.58957L3.34834 7.74737C3.5075 7.90904 3.72305 7.99915 3.94872 7.99915C3.95377 7.99915 3.95967 7.99915 3.96473 8C4.19714 7.99495 4.4161 7.89558 4.57186 7.7238L10.3072 1.40821C10.6197 1.06292 10.5944 0.530739 10.25 0.218319Z" fill="#98B64B"/>
              ...
              ${this.options?.exclusiveDiscount}
            </p>
          </div>
          <div class="group-option-item">
            <p>              
...
<path d="M10.25 0.218319C9.9073 -0.0932525 9.37427 -0.0688344 9.06104 0.275563L3.92434 5.93183L1.44273 3.40898C1.11515 3.0772 0.582966 3.07215 0.25203 3.39888C-0.079755 3.72476 -0.0848087 4.25779 0.241923 4.58957L3.34834 7.74737C3.5075 7.90904 3.72305 7.99915 3.94872 7.99915C3.95377 7.99915 3.95967 7.99915 3.96473 8C4.19714 7.99495 4.4161 7.89558 4.57186 7.7238L10.3072 1.40821C10.6197 1.06292 10.5944 0.530739 10.25 0.218319Z" fill="#98B64B"/>
  ...
  getPriceAmount(priceText) {
    return parseFloat(priceText.replaceAll("$", ""));
  }

  showDetails() {
    return html`
      <details class="skio-details" @mouseover=${(e) =>
        this.detailsMouseover()} @mouseleave=${(e) => this.detailsMouseleave()}>
        <summary>
          <span>
            <svg
  ...
  render() {
    ...
    return keyed(
      ...
      <h4 class="subscription-heading">Choose Purchase Type:</h4>
      <fieldset class="skio-plan-picker" role="radiogroup" aria-labelledby="skio-plan-picker-legend">
        <legend id="skio-plan-picker-legend" class="${
          !this.options?.show_legend ? "sr-only" : ""
        }">${unsafeHTML(this.options?.legend_content)}</legend>
        
${
  !this.product.requires_selling_plan
    ? html`
              <div
                class="group-container one-time-purchase ${
                    ...
                    <div class="skio-price" aria-live="polite">
                      ${
                        ...
                        this.selectedVariant?.compare_at_price >
                          ...
                          ? html`<s aria-hidden="true">${this.money(
                              this.selectedVariant.compare_at_price
                            )}</s>`
                          : ""
                      }
        ...
        ${
          this.availableSellingPlanGroups?.length
            ? this.availableSellingPlanGroups.map(
                (group, index) => html`
                <div class="group-container selling-plan ${
                      ...
                      <div class="group-title">
                        ...
                        ${
                          this.discountText(group.selected_selling_plan)
                            ? html` <span class="savings ${
                                this.options?.discount_style
                              }"> ${this.discountText(
      ...
      this.cart?.items.reduce(
        (aggregate, item) =>
          rule?.productVariantIds.some(
            (gid) => gid.includes(item.id) && item.selling_plan_allocation
          )
  ...
  updateExternalPrice() {
    ...
    document
      ...
      .forEach((el) => {
        this.selectedSellingPlan
          ? (el.innerHTML =
              this.price(this.selectedSellingPlan) + " " + this.currency)
          : (el.innerHTML =
              this.money(this.selectedVariant.price) + " " + this.currency);
      });
    document
      .querySelectorAll(this.options?.button_add_to_cart_selector)
      .forEach((el) => {
        el.innerHTML =
          window.fmData.variantStrings.addToCart +
          " • " +
          (this.selectedSellingPlan
